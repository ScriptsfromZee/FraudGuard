...
st.set_page_config(
    ...
    page_icon=PAGE_ICON,
    layout="centered",
    initial_sidebar_state="expanded",   # keeps sidebar open; toggle still works
)

# ──────────────────────────────────────────────────────────────────────────────
# CSS — loaded non-fatally; a missing file shows a warning, not a crash
# ──────────────────────────────────────────────────────────────────────────────
def load_css(path: str) -> None:
    if not os.path.exists(path):
        st.warning(f"⚠️  CSS file not found: {path}  — running with default styling.")
        return
    try:
        with open(path, "r", encoding="utf-8") as fh:
            st.markdown(f"<style>{fh.read()}</style>", unsafe_allow_html=True)
    except Exception as exc:
        st.warning(f"⚠️  CSS failed to load: {exc}")
...
# ──────────────────────────────────────────────────────────────────────────────
# SYSTEM PROMPT
# Full Nigerian-aware prompt — do not strip this down.
# Every constraint and every Nigerian brand reference exists for a reason.
# ──────────────────────────────────────────────────────────────────────────────
SYSTEM_PROMPT = """You are Jojo — an AI-powered fraud detection and awareness assistant
...
# Keeps the Groq API payload small enough to stay within the free-tier
# token-per-minute limit. Full history is still shown on screen.
...
# ──────────────────────────────────────────────────────────────────────────────
# SESSION STATE
# BUG FIX: previously used `if "messages" not in st.session_state` alone.
# After pressing "Clear session", the key exists but the list is empty,
# so the greeting never reappeared. Now we check both conditions.
# ──────────────────────────────────────────────────────────────────────────────
...
# ──────────────────────────────────────────────────────────────────────────────
# LOGO HELPERS
# Build the HTML for logo images with a clean emoji fallback.
# ──────────────────────────────────────────────────────────────────────────────
def logo_img(css_class: str, alt: str = "Jojo") -> str:
    """Return an <img> or an emoji <span> depending on logo availability."""
    if LOGO_SRC:
        return f'<img src="{LOGO_SRC}" class="{css_class}" alt="{alt}"/>'
    return f'<span class="{css_class} logo-emoji">🛡️</span>'
...
with st.sidebar:
    ...
    <div class="sb-brand">
      ...
      <div>
        ...
        <div class="sb-tagline">Fraud Detection</div>
      </div>
    </div>
    ...
    selected_model_id: str = MODELS[selected_model_name]
    ...
    if st.button("↺  Clear session", use_container_width=True):
        # Reset to empty list — session-state guard above re-adds the greeting
    ...
    <div class="sb-footer">
      ...
      Powered by Groq AI · Always free<br>
...
# ──────────────────────────────────────────────────────────────────────────────
# HERO
# BUG FIX: previous version used a hardcoded external CDN URL.
# Now uses LOGO_SRC consistently, with emoji fallback via logo_img().
# ──────────────────────────────────────────────────────────────────────────────
st.markdown(f"""
<div class="hero">
  <div class="hero-glow"></div>
  {logo_img("hero-logo", "Jojo")}
  <div>
    <div class="hero-title">Jojo<span class="hero-accent">.</span></div>
    <div class="hero-sub">
...
# ──────────────────────────────────────────────────────────────────────────────
# CHAT HISTORY — renders full history on screen every rerun
# ──────────────────────────────────────────────────────────────────────────────
...
if prompt := st.chat_input("Paste or describe a suspicious message, call, or offer…"):
    ...
    # BUG FIX: initialise reply before try/except so it is always defined
    reply: str = ""

    with st.chat_message("assistant"):
        with st.spinner("Scanning…"):

            # Only send the most recent messages to the API.
            # Prevents Groq free-tier token-per-minute limit at ~message 17.
            ...
            try:
                ...
                # BUG FIX: .content can be None — guard with fallback string
                if (
                    response
                ...
                if "rate_limit" in err or "429" in err:
                    reply = (
                        ...
                        "You can also switch to **Llama 3.1 8B** in the sidebar — "
                        "it has a higher free-tier request limit."
                    )
                    ...
                    reply = (
                        f"The selected model (`{selected_model_id}`) is not available. "
