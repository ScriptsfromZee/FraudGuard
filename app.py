...
# Keeps the Gemini API payload small enough to stay within the free-tier
...
if prompt := st.chat_input("Paste or describe a suspicious message, call, or offer…"):
    ...
    with st.chat_message("assistant"):
        with st.spinner("Scanning…"):
            ...
            # Prevents hitting the Gemini free-tier token-per-minute limit
            ...
            try:
                ...
                # and the system prompt is a config field, not a message role.
                gemini_contents = [
                    {
                ...
                # BUG FIX: .text can be empty (e.g. safety block) — guard with fallback
                try:
                    reply = response.text
                except Exception:
                    reply = None
...
# ──────────────────────────────────────────────────────────────────────────────
# FOOTER
# Everything that used to live in the sidebar (scan types, emergency contacts,
# session controls, credit line) now lives here, below the conversation.
# ──────────────────────────────────────────────────────────────────────────────
...
footer_msg_count: int = sum(
    1 for m in st.session_state.messages if m["role"] == "user"
)
col1, col2 = st.columns([3, 1])
with col1:
    st.markdown(
        f'<p class="sb-meta">Session scans: <strong>{footer_msg_count}</strong></p>',
        unsafe_allow_html=True,
    )
with col2:
